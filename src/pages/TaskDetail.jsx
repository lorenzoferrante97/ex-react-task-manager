...
export default function TaskDetail() {
  ...
  const { tasks, removeTasks, isTaskDeleted, setIsTaskDeleted } = useApiContext();
  ...
  if (activeTask) {
      ...
      <>
        ...
        <div className="w-full bg-neutral-100 p-3 grid grid-cols-12 gap-y-2 rounded-2xl">
          <div className="col-span-full bg-white p-4 rounded-lg flex f gap-3 items-center justify-between">
            ...
            <div>
              <button onClick={() => removeTasks(taskId)} className="hover:cursor-pointer bg-red-700 text-white rounded-lg px-4 py-2 font-semibold">
                Elimina Task
              </button>
            </div>
          </div>
    ...
    return (
      <>
        {console.log('isTaskDeleted: ', isTaskDeleted)}
